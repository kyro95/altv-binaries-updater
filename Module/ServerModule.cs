...
public class ServerModule : ICdnModule
{
    private readonly CdnInfo _cdnInfo;
    private readonly DownloaderService _downloaderService;

    public ServerModule(IOptions<CdnInfo> cdnInfo, DownloaderService downloaderService)
    {
    ...
    public List<string> GetFileList(UserPreferences userPreferences)
    {
        ...
        {
            $"{basePath}/altv-server{_downloaderService.GetOsFileExtension(userPreferences.Os)}",
            $"{basePath}/altv-crash-handler{_downloaderService.GetOsFileExtension(userPreferences.Os)}",
        };
    }

    public string GetBasePath(UserPreferences userPreferences)
    {
        return $"{_cdnInfo.BaseUri}server/{userPreferences.Branch}/{userPreferences.Os}";
    }
}
